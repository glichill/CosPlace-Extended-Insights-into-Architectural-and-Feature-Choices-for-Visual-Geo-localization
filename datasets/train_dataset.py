...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, args, dataset_folder, M=10, alpha=30, N=5, L=2,
                 current_group=0, min_images_per_class=10, use_classes=True):
        """
        ...
        if not use_classes:
            self.images = glob(os.path.join(dataset_folder, "*.jpg"))
            return
    ...
    def __getitem__(self, class_num):
       if hasattr(self, 'images'):
          # If use_classes is False, just return the image without a label
          image_path = self.images[index]
          image = open_image(image_path)
          return T.Compose([T.Resize((args.image_size, args.image_size)),
                              T.ToTensor(),
                              T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])(image), -1
