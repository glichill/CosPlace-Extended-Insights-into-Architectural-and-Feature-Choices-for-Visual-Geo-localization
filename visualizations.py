...
def write_labels_to_image(labels=["text1", "text2"]):
    ...
    font = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSerif-BoldItalic.ttf", FONTSIZE)
...
def visualize_correct_queries(eval_ds, output_folder, interested_queries):
    """Save only queries that are in the list of interested_queries."""
    os.makedirs(f"{output_folder}/saved_correct_queries", exist_ok=True)
    for query_index in interested_queries:
        query_path = eval_ds.queries_paths[query_index]
        ...
        save_path = os.path.join(output_folder, "saved_correct_queries", f"{query_index:03d}.jpg")
        img.save(save_path)

def visualize_incorrect_queries(eval_ds, output_folder, interested_queries):
    """Save only queries that are in the list of interested_queries."""
    os.makedirs(f"{output_folder}/saved_incorrect_queries", exist_ok=True)
    for query_index in interested_queries:
        query_path = eval_ds.queries_paths[query_index]
        ...
        # Save the image to the designated folder
        save_path = os.path.join(output_folder, "saved_incorrect_queries", f"{query_index:03d}.jpg")
        img.save(save_path)        
